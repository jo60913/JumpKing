...
public class Character : MonoBehaviour {
	...
	private float _speed;	//記錄血量
	private UISlider bloodHUD;	//頭頂血量的介面
	private UISlider bloodGui;	//角色角落的血量
	private UISlider AngryHUD;	//頭頂憤怒值的介面
	public float Angry;		//憤怒值
	public float AngryMax;	//憤怒值的最大值
	public bool AngryCheck;		//確定憤怒值是否完成因為animation腳本要讀取 所以無法設定為private
	public float BloodFlashTime;	//血條低於三分之一時 血條閃爍的時間
	private float _BloodFlashTime;	//記錄血條閃爍的時間
	private bool BloodFlashCheck;	//確定血條是否閃爍
	...
	private bool SkillObjCheck;		//確認大絕招附加特效是否開啟
	
	private int LeftRight;		//記錄移動方向為左右走
	private int UpDown;			//記錄移動方向為前後
	private int Slop;			//記錄方向為斜的方向
	
	public string plysit;		//記錄玩家狀態 因為animation腳本要讀取 所以無法設定為private
	private float CDTime;		//角色僵直的時間
	private float MoveDelayTime;	//角色大跳小跳落地後僵直的時間
	
	private GameObject Panel;	//角色介面的總成物件
	private UISprite HeadImg;	//角色遊戲畫面角落的頭標圖示介面
	private UISprite JumpImg;	//角色遊戲畫面角落的跳躍圖式介面
	private UISprite BigJumpImg;	//角色遊戲畫面角落的大跳躍圖式介面
	private UISprite SkillImg;	//角色遊戲畫面角落的突進圖示介面
	private UISprite JumpCDImg;	//角色遊戲畫面角落的跳躍CD遮罩圖式介面
	private UISprite BigJumpCDImg;	//角色遊戲畫面角落的大跳躍CD遮罩圖式介面
	private UISprite SkillCDImg;	//角色遊戲畫面角落的突進CD遮罩圖式介面
	
	private GameObject Damage;	//角色頭頂傷害介面總成
	private UISprite DamageOneImg;	//角色頭頂傷害數字的個位數
	private UISprite DamageTenImg;	//角色頭頂傷害數字的十位數
	private UISprite DamageHundImg;		//角色頭頂傷害數字的百位數
	private UISprite Negative;	//角色頭頂傷害的符號 扣血的話會顯示- 加血的話會顯示+
	public float JumpCD;	//角色小跳躍CD時間
	private float _JumpCD;	//記錄角色小跳躍CD時間
	public float BigJumpCD;	//角色大跳躍CD時間
	private float _BigJumpCD;	//記錄角色大跳躍CD時間
	public float SkillCD;	//角色突進CD時間
	private float _SkillCD;	//記錄角色突進CD時間
	
	private bool SkillDamCheck;	//確認現在角色是否可以被突進攻擊並扣血 (避免Unity對角色重複扣血)
	private bool JumpCheck;		//確認現在角色是否可以釋放小跳絕招
	private bool BigJumpCheck;	//確認現在角色是否可以釋放大跳絕招
	private bool SkillCheck;	//確認是否可以釋放突進攻擊
	
	private bool DieCheck;		//死亡確認 用來確認死亡音效的播放 避免重複播放
	...
	private string AttName;			//記錄被攻擊的敵人編號 角色死亡後會依這個變數記錄的數字 把資料傳送給GameManager記錄殺敵數
	private GameManager _GameManager;	//宣告用來呼叫GameManager腳本
	public GameObject gamemamager;		//宣告用來呼叫GameManager物件
	private GameObject HUD;		//角色頭頂介面總成
	
	public GameObject WaAtt;	//受到傷害的粒子特效
	public GameObject DieEffect;	//死亡後會產生的特效
	
	public int EleLeft;		//角色麻痺時左鍵所要按的次數
	private int _EleLeft;	//記錄左鍵所要按的次數
	public int EleRight;	//角色麻痺時右鍵所要按的次數
	private int _EleRight;	//記錄右鍵所要按的次數
	private bool EleCheck;	//確認現在是該按下左鍵還是右鍵 避免玩家抓到Bug只要按同一個按鍵就可以
	
	public float JumpTime;	//跳躍的時間 從離開地面到降落地面
	private float _JumpTime;	//記錄跳躍的時間
	public float Heigh;		//跳躍的高度 1個單位的話等於Unity一個Cube的高度 (Cube的Scale X Y Z 都要等於1)
	private float _Heigh;	//記錄跳躍的高度
	public float JumpDis;	//跳躍的距離 1個單位等於Unity一個Cube的長度 (Cube的Scale X Y Z 都要等於1)
	private float _JumpDis;	//記錄跳躍的距離
	private float Gravity;	//地吸引力 因為跳躍的控制是方程式控制 所以要寫一個地吸引力做為模擬
	
	public float BigJumpTime;	//大跳躍的時間 從離開地面到降落地面
	private float _BigJumpTime;	//記錄大跳躍的時間
	public float BigJumpHeigh;		//大跳躍的高度 1個單位的話等於Unity一個Cube的高度 (Cube的Scale X Y Z 都要等於1)
	private float _BigJumpHeigh;	//記錄大跳躍的高度
	public float BigJumpDis;	//大跳躍的距離 1個單位等於Unity一個Cube的長度  (Cube的Scale X Y Z 都要等於1)
	private float _BigJumpDis;	//記錄大跳躍的距離
	private float BigJumpGravity;	//地吸引力 因為跳躍的控制是方程式控制 所以要寫一個地吸引力做為模擬
	
	public float SkillDis;	//突進的距離 1個單位的話等於Unity一個Cube的高度 (Cube的Scale X Y Z 都要等於1)
	public float SkillTime;	//突進的時間
	private float _SkillTime;	//記錄突進的時間
	
	private Vector3 lastPosition;	//記錄最後的位置 如果跑出場外的時候會把角色拉回這個位置
	private bool AttCheck;	//記錄可以被攻擊地板攻擊 用來避免腳本從覆扣血
	...
	void OnCollisionEnter(Collision other) 			
	{
		if(other.gameObject.tag == "Out")	//如果碰到界線的物件時
		{
			...
			if(plysit == "bigjump")				//如果為大跳躍狀態時 (表示跳躍的時候碰到界線物件)
			{
				...
				gamemamager.SendMessage("BigJumpSkillImgCha",1);	//傳送訊息給GameManager 讓GameManager在遊戲畫面出現蝴蝶的圖片
