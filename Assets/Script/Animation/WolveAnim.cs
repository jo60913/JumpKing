...
public class WolveAnim : MonoBehaviour {

	private Animator anim;
	public GameObject mat;
	private WolveMove _WolveMove;
	...
	void Start () {
		...
		CDTime = 3;
		SkillTime = 0.7f;
		anim = transform.Find("Roles_04_1008_TPossSkin").GetComponent<Animator> ();
		_WolveMove = GetComponent<WolveMove>();
		Start_Eff.SetActive(false);
		Mid_Eff.SetActive(false);
		LastBlood = _WolveMove.blood;
		CurrentBlood = _WolveMove.blood;
	...
	void Update () {
		...
		if(_WolveMove.plysit == "CDTime")						/////時間buff變黑
		{
			CDtimeCheck = true;
		}
		...
		if(_WolveMove.plysit == "Skill")						/////放衝刺變青澀
		{
			SkillCheck = true;
		}
		...
		if(_WolveMove.plysit == "jumpingpar")						
		{
			anim.SetFloat("Speed",0);
		...
		if(_WolveMove.plysit == "bigjumppar")						
		{
			anim.SetFloat("Speed",0);
		...
		if(_WolveMove.plysit == "ground")
		{
			if(Input.GetAxis(_WolveMove.PlyHor) == 1 || Input.GetAxis(_WolveMove.PlyHor) == -1)	//移動
			{
				anim.SetFloat("Speed",1);
			}
			
			if(Input.GetAxis(_WolveMove.PlyVer) == 1 || Input.GetAxis(_WolveMove.PlyVer) == -1)
			{
				anim.SetFloat("Speed",1);
			}
		}
		
		if(Input.GetAxis(_WolveMove.PlyHor) == 0)			//////放開
		{
			if(Input.GetAxis(_WolveMove.PlyVer) == 0)
			{
				anim.SetFloat("Speed",0);
			}
		}
	...
	IEnumerator InvincibleTime()                    
	{
		...
		yield return new WaitForSeconds(_WolveMove.InvincibleTime);
		InvincibleObj.SetActive(false);
